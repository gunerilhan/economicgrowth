...
# subtitle: ""
subtitle: "Lecture 3: The Solow Growth Model, cont'd <br><html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>"
author: '&#x130;lhan G&#252;ner'
...
Iterate to get $k(t)$ and $y(t)$ for $t=3,4,...$
]
...
- <u>**Convergence toward the steady state:**</u> A country's per-worker output will grow or shrink from some initial position toward the steady-state level determined by the investment rate.
---
.pull-left[
### Steady State
...
- Therefore, consumption at the steady-state is equal to 

$$ 
c^\ast = (1-\gamma) y^\ast = (1-\gamma)A^{1/(1-\alpha)}\left(\frac{\gamma}{\delta+n} \right)^{\alpha/(1-\alpha)}h
$$
...
### Output per worker increases after an increase in the investment rate
...
### Growth Rates
.pull-left[
- The Solow model predicts that growth is faster when an economy is farther away from steady state. 
...
| Variable | Notation |Steady state growth rate |
|:---|:---:|:---:|
| Capital per worker | $k$ | $0$ |
| Output per worker | $y$ | $0$ |
| Consumption per worker | $c$  | $0$ |
| Capital | $K$ | $n$ |
| Output | $Y$ | $n$ | 
| Consumption | $C$ | $n$ | 
...
- 2019 output per worker levels

- Investment rate is average over 1970-2019.
]
...
<center>
<img src="./files/rel_GDP_pred_inv.svg" width="75%"/>
</center>
Data source: Penn World Tables, version 10.0

Correlation of predicted vs actual values: 0.52
]
...
- Employment growth rate is average over 1970-2019.
]
...
<center>
<img src="./files/rel_GDP_pred_inv_emp.svg" width="75%"/>
</center>
Data source: Penn World Tables, version 10.0

Correlation of predicted vs actual values: 0.64
]
...
- Human capital is the 2019 level.
]
...
<center>
<img src="./files/rel_GDP_pred_inv_emp_hc.svg" width="75%"/>
</center>
Data source: Penn World Tables, version 10.0

Correlation of predicted vs actual values: 0.77
]
...
- In the Solow model, countries accumulate capital per worker and converge to steady state over time.

- Investment generates capital accumulation, and hence higher levels of output per worker.


- In the steady state, capital per worker, output per worker, and consumption per worker are constant.

- There is only short-run economic growth in the Solow model mainly because of the diminishing marginal product of capital.

- Despite the parsimonious nature of the Solow model, it predicts relative incomes quite well.
