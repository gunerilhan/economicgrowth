...
$Y = F(K,L)$, where $Y$ is output, $K$ is physical capital, and $L$ is labor.

--

- Assume $F(\cdot)$ displays **constant returns to scale (CRS):** $F(zK,zL)=zF(K,L)$, where $z$ is a constant.

--

- Convert aggregate production function, $F(\cdot)$, into per worker production function, $f(\cdot)$, by multiplying  $F(\cdot)$ with $1/L$:

$$\left(\frac{1}{L} \right)Y = \left(\frac{1}{L} \right) F(K,L)  = F\left(\frac{K}{L},\frac{L}{L} \right) = F\left(\frac{K}{L},1 \right)$$

--
- Define $k \equiv K/L$ as capital per labor and $y \equiv Y/L$ as output per labor:
...
where $A$ is productivity, $K$ is capital, and $L$ is labor, $0 < \alpha < 1$.
...
$$ \frac{Y}{L} = \frac{ A K^\alpha (L)^{1-\alpha}}{L} = A \frac{K^\alpha}{L^\alpha}\frac{L^{1-\alpha}}{L^{1-\alpha}} $$
...
$$ \text{MPK} = \frac{\partial y}{\partial k} = \alpha A k^{\alpha-1} > 0$$


- Diminishing marginal product of capital

$$ \frac{\partial^2 y}{\partial k^2} = (\alpha-1)\alpha A k^{\alpha-2} < 0 $$
...
- Qualities of labor are the same across countries.  _Will be relaxed in later lectures_
...
<center>
<img src="./files/capital-output-simul.svg" width="60%"/>
</center>
...
- Derived the law of motion for physical capital stock

- Learned convergence to the steady state
...
#### To prepate for the next lecture 
